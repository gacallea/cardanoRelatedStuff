...
EXPORTER_PORT = int(os.getenv("PORT", "9101"), 10)
SLEEP_TIME = int(os.getenv("SLEEP_TIME", "10"), 10)
JORMUNGANDR_API = os.getenv(
    "JORMUNGANDR_RESTAPI_URL",
    os.getenv(
        "JORMUNGANDR_API", "http://127.0.0.1:<REST_API_PORT>/api"
    ),  # change this to your REST API port
)
...
jormungandr_funds = Gauge(
    f"jormungandr_address_funds",
    f"Jormungandr Address funds in Lovelace",
    ["addr"],
)
jormungandr_counts = Gauge(
    f"jormungandr_address_counts", f"Jormungandr Address counter", ["addr"]
)

to_reset = [jormungandr_metrics, jormungandr_pieces]

to_reset_with_labels = [jormungandr_funds, jormungandr_counts]
...
def process_jormungandr_metrics():
    ...
    try:
        ...
        for i, (x, y) in enumerate(
            list(zip(range(-64, 8, 8), range(-56, 8, 8))), 1
        ):
            if y == 0:
                y = None
            blockHashPieces["lastBlockHashPiece" + str(i)] = int(
                lastBlockHashProcess[slice(x, y)], 16
            )
